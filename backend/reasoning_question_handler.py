...
import operator
from structured_output_classes import GradeDocument
from prompts import (
...
class ReasoningQuestionHandler:
    ...
    def _grade_document(self, state: State):
        ...
        Response: {GradeDocument(result='NO').model_dump_json()}
        ...
        Response: {GradeDocument(result='YES').model_dump_json()}
        ...
        chain = prompt | self.llm.with_structured_output(GradeDocument, method='json_mode')
        result = chain.invoke(
            {"question": query, "document": state["document"], "examples": examples}
        )
        if  "YES" in result.result or state["max_retries"] <= 0:
    ...
    def _should_continue(self, state: State):
        ...
        if "YES" in result.result  or state["max_generations"] <= 0:
